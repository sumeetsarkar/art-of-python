...
class Github:
  ...
  def __parse_response(self, response, encoding):
    ...
    print('Total search results: ' + str(responseJSON['total_count']) + '\n')
    for repo in responseJSON.get('items'):
      print(repo['full_name'])
      print(repo['html_url'])
      print('Author: ' + repo['owner']['login'])
      print('stars: {0} forks: {1} issues: {2}'.format(str(repo['stargazers_count']), str(repo['forks']), str(repo['open_issues'])))
      print('----------------------------\n')

  def search_repo(self, repoName):
    myreq = request.Request(method='GET', url=self.__get_request_url(repoName))
    with request.urlopen(myreq) as r:
      print(r.status, r.reason)
      print('\nResponse Headers')
      print('----------------\n')
      print(r.headers)
      self.__parse_response(r.read(), 'utf8')
